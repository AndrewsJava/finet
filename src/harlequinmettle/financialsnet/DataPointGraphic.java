...
public class DataPointGraphic {
	...
	private void init(String category2, String ticker2) {
		...
		if (general) {
			graphicRank += heightFactor * (rank + 1);
		...
		border = new Rectangle2D.Float(PIXELS_BORDER, PIXELS_BORDER + rank
				* PIXELS_BORDER + rank * PIXELS_HEIGHT, ProfileCanvas.W - 2
				* PIXELS_BORDER, PIXELS_HEIGHT * heightFactor);
	}
	...
	private void setMinMaxHistogramHighlight() {
		...
		if (min == min)
			minMaxHisto.x =	Database.statistics.get(categoryId).locationInHistogram(min);
		else
			minMaxHisto.x = -10;

		if (max == max)
			minMaxHisto.y =	Database.statistics.get(categoryId).locationInHistogram(max);
		else
			minMaxHisto.y = -10;
	...
	private void setUpTechnicalsGraphData() {
		...
		TreeMap<Float, float[]> technicals  = Database.TECHNICAL_PRICE_DATA.get(Database.dbSet.get(id));
...
public void drawMe(Graphics2D g){
 ...
 int J = 0;
		...
		g.draw(border);
			...
			g.drawString("" +minMaxLine.x, ProfileCanvas.W - 100, top+PIXELS_HEIGHT-PIXELS_BORDER);
